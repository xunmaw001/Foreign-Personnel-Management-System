<template>
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback" :bottom="100">
		<view class="container">
					...
					<view class="lable" :style='{"width":"auto","fontSize":"28rpx","color":"rgba(0, 102, 51, 1)","textAlign":"left"}'>人口移送：</view>
					<view class="text" :style='{"color":"rgba(0, 102, 51, 1)","textAlign":"center","fontSize":"24rpx"}'>{{detail.renkouyisong}}</view>
				</view>
...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','renkouyisong');
			...
			onAcrossTap(tableName){
     			uni.setStorageSync('crossTable','renkouyisong');
				uni.setStorageSync(`crossObj`, this.detail);
				this.$utils.jump(`../${tableName}/add-or-update?cross=true`);
			},
			// 获取详情
			async init(){
				let res = await this.$api.info('renkouyisong', this.id);
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discussrenkouyisong/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('renkouyisong', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('renkouyisong', this.detail);
				this.$utils.msg('审核成功');
